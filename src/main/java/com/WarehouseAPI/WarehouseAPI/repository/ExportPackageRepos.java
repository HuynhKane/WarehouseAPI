...
import com.WarehouseAPI.WarehouseAPI.dto.ExportPackageInfo;
...
public interface ExportPackageRepos extends MongoRepository<ExportPackage, String> {
    ...
    @Aggregation(pipeline = {
            "{ $match: { statusDone: 'APPROVED', exportDate: { $gte: ?0, $lt: ?1 } } }",
            "{ $project: { _id: 1, packageName: 1, exportDate: 1, totalSellingPrice: { $toDouble: '$totalSellingPrice' } } }",
            "{ $sort: { totalSellingPrice: -1 } }"
    })
    List<ExportPackageInfo> getExportPackagesByMonth(Date startDate, Date endDate);
}
